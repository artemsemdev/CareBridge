...
**Status:** All 3 issues (A6-01 through A6-03) implemented. Audit Service wired with in-memory audit store (`IAuditStore` / `InMemoryAuditStore`), event consumers for all 14 domain event types, centralized event-to-audit mapping (`AuditEventMapper`), EventId-based idempotency, 5-retry policy for audit events. Read-only query API (`GET /api/v1/audit` with combinable filters, `GET /api/v1/audit/{recordId}` with full deserialized payload). No mutation endpoints exist. Gateway extended with AuditService HTTP client and 3 BFF proxy endpoints. React frontend updated with standalone audit log page (`/audit`) with full-width table, event type/entity type/actor/case/date range filters, expandable row detail with formatted JSON payload, cursor pagination, and case link navigation. Case detail page extended with "Recent Activity" section showing last 10 audit entries and "View full audit trail" link. Shared eventing extended with configurable `MaxRetries` on `EventConsumerOptions` (default 3, backwards-compatible). 140 unit tests pass (18 contract + 3 case + 19 threshold + 18 task + 11 appointment + 27 reporting + 44 audit).

### Stage 6: Hardening and Observability (estimated: ~1 week) — COMPLETED 2026-04-02
...
**Status:** All 5 issues (H-01 through H-05) implemented. Health endpoints extended with dependency checks: SQL Server health checks on all 7 database services, RabbitMQ health checks on all 6 event-consuming services, via `AspNetCore.HealthChecks.SqlServer` and `AspNetCore.HealthChecks.Rabbitmq` packages in shared infrastructure. Structured JSON logging standardized across all services via Serilog `RenderedCompactJsonFormatter` with service name enrichment, correlation ID propagation, and request logging at Debug level via `UseSerilogRequestLogging()`. OpenTelemetry distributed tracing integrated with ASP.NET Core, HttpClient, and EF Core instrumentation, W3C TraceContext propagation, console exporter for local dev, and configurable enable/disable flag. Circuit breakers added to all 10 inter-service HTTP clients (8 in Gateway, 1 in CareGap Engine, 1 in CarePlan Service) via `Microsoft.Extensions.Http.Resilience` standard resilience handler: 2 retries with exponential backoff, circuit breaker (50% failure threshold, 30s break), 10s attempt timeout. Error handling hardened: `EnableRetryOnFailure()` on all 7 DbContexts, input length validation on Case Service, `DbUpdateException` mapped to 409 in `ExceptionHandlerMiddleware`, `BrokenCircuitException` mapped to 503 in Gateway proxy. 140 unit tests pass. See ADR-012.
